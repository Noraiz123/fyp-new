...
function Login() {
    ...
    const verifyCode = async (actionCode) => {
      auth
        ...
        .then((res) => {
          console.log(res);
          setSuccess(true);
          setFormError('Your email is verified successfully. Login again to complete the verification process.');
          if (auth.currentUser) {
  ...
  const signIn = async (email, password) => {
    dispatch(StartLogin({ identifier: email, password }))
    // .then(() => {
    //   setSuccess(true);
    //   setFormError('Success! Redirecting...');
    //   setEmail('');
  ...
  return (
    <div className='login'>
      <form className='form' onSubmit={(e) => handleSubmit(e)}>
        ...
        <Link to={query.get('next') ? `/password-reset?next=${query.get('next')}` : '/password-reset'}>
        ...
        <div className='form__element buttons'>
          <button
            disabled={status}
            onClick={() => history.replace(query.get('next') ? `/signup?next=${query.get('next')}` : '/signup')}
