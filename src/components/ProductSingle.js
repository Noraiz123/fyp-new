...
function ProductSingle() {
  ...
  const [{ cart, bookmarks, loadingBar }, dispatch] = useStateValue();
  ...
  return (
    <div className='productSingle'>
      ...
      <div className='productSingle__suggested'>
        <h5 style={{ marginTop: '3rem', marginBottom: '1rem' }}>You might also like</h5>
        <div className='products'>
          {suggestions
            ?.slice(0, 3)
            .map((product) => (product.id != id ? <Product id={product.id} item={product} /> : null))}
        </div>
      </div>
    </div>
  );
