...
import { StartLogin, StartSignup } from '../redux/actions/authActions';
...
function Signup() {
  ...
  const signUp = async (username, email, password) => {
    ...
    //           setSuccess(true);
    //           loadingBar.current.complete();
    //           setFormError('Verify your email to complete registration. Check your inbox/spam for verification email.');
    //           setStatus(false);
    //           setTimeout(() => {
    //             if (!query.get('next')) {
  ...
  return (
    <div className='signup'>
      <form className='form' onSubmit={(e) => handleSubmit(e)}>
        ...
        {!success && (
          <motion.div initial='initial' animate='in' exit='out' variants={errorAnim}>
            <div className='form__element'>
              <input
                className={`${name ? 'active ' : ''} ${errorName ? 'error ' : ''}`}
                id='name'
                type='text'
                value={name}
              ...
              {errorName && (
                <motion.p initial='initial' animate='in' exit='out' variants={errorAnim} className='error'>
            ...
            <div className='form__element'>
              <input
                className={`${email ? 'active ' : ''} ${errorEmail ? 'error ' : ''}`}
                id='email'
                type='text'
                value={email}
            ...
            <div className='form__element'>
              <input
                className={`${password ? 'active ' : ''} ${errorPassword ? 'error ' : ''}`}
                id='password'
            ...
            <div className='form__element buttons'>
              <button disabled={status} className='button buttonPrimary submit' type='submit'>
            ...
            <div className='form__element buttons'>
              <button
                onClick={() => history.replace(query.get('next') ? `/login?next=${query.get('next')}` : '/login')}
                disabled={status}
                className='button buttonSecondary signup'
              >
                Login In, instead
              </button>
              <button onClick={googleSignIn} disabled={status} className='button buttonSecondary outline'>
